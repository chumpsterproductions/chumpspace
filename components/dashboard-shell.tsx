      ...
      <Dialog open={showUserSettings} onOpenChange={setShowUserSettings}>
        <DialogContent>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0"><p className="truncate font-medium">{data.profile.full_name ?? "Discord user"}</p><p className="truncate text-sm text-primary">@{getProfileHandle(data.profile)}</p><p className="private-email truncate text-sm text-muted-foreground">{data.profile.email}</p></div>
          </div>
